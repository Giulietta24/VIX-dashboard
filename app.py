...
if status == "OK":
    ...
    # Metrics with dynamic unit formatting
    ...
    # Price Line (White - clearly visible on Dark Template)
    ...
    fig.update_layout(template="plotly_dark", height=500, hovermode="x unified",
                      paper_bgcolor="#111111", plot_bgcolor="#111111", # Forces dark background
                      legend=dict(orientation="h", y=1.1))

    fig.update_yaxes(title_text="Price ($)", secondary_y=False, showgrid=False)
    fig.update_yaxes(title_text="Capital Flow ($)", secondary_y=True, tickformat="~s", showgrid=True)
    ...
    fig2.update_layout(template="plotly_dark", height=350, paper_bgcolor="#111111", plot_bgcolor="#111111")
