...
st.set_page_config(page_title="VIX Conviction Terminal", layout="wide")
ticker = st.sidebar.text_input("Enter Ticker", "HIMS").upper()

# --- 2. DATA ENGINE ---
@st.cache_data(ttl=3600)
def get_vix_data(symbol):
    try:
...
status, df, spy_price = get_vix_data(ticker)
...
if status == "OK":
    ...
    # --- CHART 2: FEAR GAUGE (Fixing the Error Location) ---
    ...
    if not np.isnan(df['Whale_Buy'].iloc[-1]):
        ...
        st.info("⚖️ **NEUTRAL:** No major Whale activity today.")

elif status == "LIMIT":
    st.error("🚨 API Limit Reached (25/day). Refresh tomorrow or swap key.")
else:
